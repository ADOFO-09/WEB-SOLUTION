...
<div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
    ...
    <div>
        ...
        @if(isset($ministry) && $ministry)
        @php $dashGroups = $ministry->activeGroups()->withCount('members')->orderBy('name')->get(); @endphp
        @if($dashGroups->count() > 0)
        <!-- Sub-Groups -->
        <div class="card mb-6">
            <div class="card-header" style="display: flex; justify-content: space-between; align-items: center;">
                <h3 style="font-weight: 600; color: #1e3a5f;">Sub-Groups</h3>
                <a href="{{ route('admin.ministries.groups.index', $ministry) }}"
                   style="font-size: 0.75rem; color: #4f46e5; font-weight: 500;">Manage</a>
            </div>
            <div class="card-body" style="padding: 0.5rem 1rem;">
                @foreach($dashGroups as $dg)
                <a href="{{ route('admin.ministries.groups.report', [$ministry, $dg]) }}"
                   style="display: flex; align-items: center; justify-content: space-between; padding: 0.5rem 0; {{ !$loop->last ? 'border-bottom: 1px solid #f1f5f9;' : '' }} text-decoration: none;">
                    <span style="font-size: 0.875rem; color: #374151;">{{ $dg->name }}</span>
                    <span style="font-size: 1rem; font-weight: 700; color: #4f46e5;">{{ $dg->members_count }}</span>
                </a>
                @endforeach
                @php $ungrouped = $ministry->totalUngroupedMembers(); @endphp
                @if($ungrouped > 0)
                <div style="display: flex; align-items: center; justify-content: space-between; padding: 0.5rem 0; border-top: 1px solid #f1f5f9;">
                    <span style="font-size: 0.875rem; color: #9ca3af; font-style: italic;">Unassigned</span>
                    <span style="font-size: 1rem; font-weight: 700; color: #d97706;">{{ $ungrouped }}</span>
                </div>
                @endif
            </div>
        </div>
        ...
        <div class="card">
            ...
            <div class="card-body">
                ...
                @if(isset($ministry) && $ministry)
                <a href="{{ route('admin.ministries.groups.index', $ministry) }}" class="btn btn-secondary" style="width: 100%; margin-bottom: 0.5rem;">
                    <svg style="width: 1rem; height: 1rem; margin-right: 0.5rem;" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
                ...
                @endif
                <a href="{{ route('admin.ministry.compose-sms') }}" class="btn btn-secondary" style="width: 100%; margin-bottom: 0.5rem;">
                    <svg style="width: 1rem; height: 1rem; margin-right: 0.5rem;" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
                        <path stroke-linecap="round" stroke-linejoin="round" d="M8.625 9.75a.375.375 0 11-.75 0 .375.375 0 01.75 0zm0 0H8.25m4.125 0a.375.375 0 11-.75 0 .375.375 0 01.75 0zm0 0H12m4.125 0a.375.375 0 11-.75 0 .375.375 0 01.75 0zm0 0h-.375m-13.5 3.01c0 1.6 1.123 2.994 2.707 3.227 1.087.16 2.185.283 3.293.369V21l4.184-4.183a1.14 1.14 0 01.778-.332 48.294 48.294 0 005.83-.498c1.585-.233 2.708-1.626 2.708-3.228V6.741c0-1.602-1.123-2.995-2.707-3.228A48.394 48.394 0 0012 3c-2.392 0-4.744.175-7.043.513C3.373 3.746 2.25 5.14 2.25 6.741v6.018z" />
                    </svg>
                    Send SMS
                </a>
                <a href="{{ route('admin.ministry.finance.index') }}" class="btn btn-secondary" style="width: 100%;">
                    <svg style="width: 1rem; height: 1rem; margin-right: 0.5rem;" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
