...
@section('title', 'Member Welfare Summary — '.$ministry->name)

@section('content')
<div class="mb-6 flex items-center justify-between">
    <div>
        <h1 style="font-size:1.5rem;font-weight:700;color:#1e3a5f;">Member Welfare Summary</h1>
        <p style="color:#64748b;margin-top:0.25rem;">{{ $ministry->name }} &mdash; contribution and benefit history per member</p>
    </div>
...
<form method="GET" style="background:#fff;border-radius:10px;padding:1rem 1.25rem;box-shadow:0 1px 4px rgba(0,0,0,0.06);margin-bottom:1.25rem;">
    <div style="display:flex;gap:0.75rem;align-items:center;">
        ...
        <button type="submit" style="background:#1e3a5f;color:#fff;padding:0.5rem 1.25rem;border-radius:6px;border:none;font-size:0.875rem;font-weight:600;cursor:pointer;white-space:nowrap;">Search</button>
        @if($search)
        <a href="{{ route('admin.ministry.finance.welfare.members') }}" style="font-size:0.82rem;color:#64748b;white-space:nowrap;">Clear</a>
        @endif
    </div>
</form>
        ...
        <table style="width:100%;font-size:0.82rem;border-collapse:collapse;">
            <thead>
                <tr style="background:#f8fafc;border-bottom:2px solid #e2e8f0;">
                    ...
                    <th style="text-align:right;padding:0.7rem 1rem;color:#64748b;font-weight:600;font-size:0.72rem;text-transform:uppercase;">Benefits</th>
            ...
            <tbody>
                ...
                <tr style="border-bottom:1px solid #f8fafc;" onmouseover="this.style.background='#fafafa'" onmouseout="this.style.background=''">
                    ...
                    <td style="padding:0.65rem 1rem;text-align:right;font-weight:600;color:#374151;">{{ $member->contrib_count ?? 0 }}</td>
                    <td style="padding:0.65rem 1rem;text-align:right;font-weight:600;color:#7c3aed;">{{ $member->benefit_count ?? 0 }}</td>
                    <td style="padding:0.65rem 1rem;text-align:right;">
                        <a href="{{ route('admin.ministry.finance.welfare.members.show', $member->id) }}"
                           style="background:#1e3a5f;color:#fff;padding:0.3rem 0.75rem;border-radius:5px;font-size:0.75rem;font-weight:600;text-decoration:none;">
                            View Records
                        </a>
                    </td>
                </tr>
                @empty
                <tr>
                    <td colspan="4" style="padding:3.5rem;text-align:center;color:#9ca3af;font-size:0.875rem;">
                        @if($search)
