            ...
            <tr>
                ...
                <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                    <div class="flex items-center justify-end space-x-3">
                        <a href="{{ route('admin.service-types.show', $type) }}" class="text-indigo-600 hover:text-indigo-900">View</a>
                        @can('attendance.edit')
                        <a href="{{ route('admin.service-types.edit', $type) }}" class="text-yellow-600 hover:text-yellow-900">Edit</a>
                        @endcan
                        @can('attendance.delete')
                        <form action="{{ route('admin.service-types.destroy', $type) }}" method="POST" class="inline">
                            @csrf
                            @method('DELETE')
                            <button type="submit"
