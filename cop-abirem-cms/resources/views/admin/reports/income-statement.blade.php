...
<div class="max-w-4xl mx-auto space-y-6">
        ...
        <div class="p-6">
            <table class="w-full">
                <tbody class="divide-y divide-gray-100">
                    <tr><td class="py-3">Tithes</td><td class="py-3 text-right font-medium">{{ $currencySymbol }} {{ number_format($income['tithes'], 2) }}</td></tr>
                    <tr><td class="py-3">Offerings</td><td class="py-3 text-right font-medium">{{ $currencySymbol }} {{ number_format($income['offerings'], 2) }}</td></tr>
                    <tr><td class="py-3">Donations</td><td class="py-3 text-right font-medium">{{ $currencySymbol }} {{ number_format($income['donations'], 2) }}</td></tr>
                </tbody>
                <tfoot>
        ...
        <div class="p-6">
            <table class="w-full">
                <tbody class="divide-y divide-gray-100">
                    ...
                    <tr><td class="py-3">{{ $expense->expenseCategory->name ?? 'N/A' }}</td><td class="py-3 text-right font-medium">{{ $currencySymbol }} {{ number_format($expense->total, 2) }}</td></tr>
                    @empty
    ...
    <div class="bg-white rounded-lg shadow-sm border p-6">
        <div class="flex justify-between items-center">
            ...
            <span class="text-3xl font-bold {{ $netIncome >= 0 ? 'text-green-600' : 'text-red-600' }}">{{ $currencySymbol }} {{ number_format($netIncome, 2) }}</span>
        </div>
    </div>
</div>
@endsection
