...
@section('content')
    ...
    <div class="bg-white rounded-lg shadow mb-6">
        <div class="p-4">
            <form action="{{ route('admin.welfare.reports.spending') }}" method="GET"
                ...
                <div>
                    ...
                    <select name="purpose" class="w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 text-sm">
                        ...
                        @foreach(\App\Models\WelfareBenefit::PURPOSES as $key => $label)
                            <option value="{{ $key }}" {{ $purpose === $key ? 'selected' : '' }}>{{ $label }}</option>
                        @endforeach
                    </select>
                </div>
                <div class="flex items-end gap-2">
                    ...
                    <a href="{{ route('admin.welfare.reports.spending') }}" class="px-3 py-2 border border-gray-300 rounded-md hover:bg-gray-50 text-sm">Clear</a>
                </div>
            </form>
        </div>
    </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
        <div class="bg-white rounded-lg shadow p-4">
            <div class="text-sm font-medium text-gray-500">Total Main Benefits</div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <table class="min-w-full divide-y divide-gray-200">
                <thead class="bg-gray-50">
                    <tr>
                        ...
                        <th class="px-4 py-3 text-right text-xs font-medium text-gray-500 uppercase">Main</th>
        ...
        <div class="bg-white rounded-lg shadow overflow-hidden">
            <div class="px-6 py-4 border-b">
                <h2 class="text-lg font-semibold text-gray-900">By Purpose</h2>
            </div>
            <table class="min-w-full divide-y divide-gray-200">
                <thead class="bg-gray-50">
                    <tr>
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    @forelse($summary['by_purpose'] as $row)
                    <tr class="hover:bg-gray-50">
                        <td class="px-4 py-3 text-sm text-gray-700">{{ $row['label'] }}</td>
                        <td class="px-4 py-3 text-sm text-right text-gray-600">{{ $currencySymbol }} {{ number_format($row['main'], 2) }}</td>
                        <td class="px-4 py-3 text-sm text-right text-orange-600">{{ $currencySymbol }} {{ number_format($row['other'], 2) }}</td>
