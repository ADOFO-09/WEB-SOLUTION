...
@section('title', 'Member Welfare Summary')

@section('content')
<div class="mb-6 flex items-center justify-between">
    <div>
        <h1 style="font-size:1.5rem;font-weight:700;color:#1e3a5f;">Member Welfare Summary</h1>
        <p style="color:#64748b;margin-top:0.25rem;">View contribution history, balances, and benefits for each member</p>
    </div>
</div>
...
<form method="GET" style="background:#fff;border-radius:10px;padding:1rem 1.25rem;box-shadow:0 1px 4px rgba(0,0,0,0.06);margin-bottom:1.25rem;">
    <div style="display:flex;gap:0.75rem;align-items:center;">
        ...
        @if($search)
        <a href="{{ route('admin.welfare.members.index') }}" style="font-size:0.82rem;color:#64748b;white-space:nowrap;">Clear</a>
        @endif
    </div>
</form>
            ...
            <tbody>
                ...
                <tr style="border-bottom:1px solid #f8fafc;transition:background 0.1s;" onmouseover="this.style.background='#fafafa'" onmouseout="this.style.background=''">
                    ...
                    <td style="padding:0.65rem 1rem;color:#64748b;font-size:0.82rem;">
                        @if($member->welfare_enrolled && $member->welfare_start_date)
                            {{ $member->welfare_start_date->format('d M Y') }}
                        @elseif($member->welfare_enrolled)
                            <span style="color:#64748b;">Enrolled</span>
                        @else
                            <span style="color:#94a3b8;font-style:italic;">Not enrolled</span>
                        @endif
                    </td>
                    <td style="padding:0.65rem 1rem;text-align:right;font-weight:600;color:#374151;">{{ $member->welfare_contributions_count }}</td>
                    ...
                    <td style="padding:0.65rem 1rem;text-align:right;font-weight:600;color:#7c3aed;">{{ $member->welfare_benefits_count }}</td>
                    <td style="padding:0.65rem 1rem;text-align:right;">
                        <a href="{{ route('admin.welfare.members.show', $member) }}"
