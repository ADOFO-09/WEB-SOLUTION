...
@section('header')
    <div class="flex items-center">
        <a href="{{ route('admin.donations.show', $donation) }}" class="mr-4 text-gray-400 hover:text-gray-600">
...
@section('content')
<form action="{{ route('admin.donations.update', $donation) }}" method="POST" class="max-w-3xl">
    @csrf
    @method('PUT')
        ...
        <div class="p-6 space-y-6">
            ...
            <div class="flex items-center">
                <input type="checkbox" name="is_anonymous" id="is_anonymous" value="1" 
                       ...
                       {{ old('is_anonymous', $donation->is_anonymous) ? 'checked' : '' }}>
            ...
            <div>
                ...
                <div class="flex space-x-4">
                    <label class="inline-flex items-center">
                        <input type="radio" name="donation_type" value="cash" class="text-indigo-600" {{ old('donation_type', $donation->donation_type) == 'cash' ? 'checked' : '' }}>
                        <span class="ml-2">Cash</span>
                    </label>
                    <label class="inline-flex items-center">
                        <input type="radio" name="donation_type" value="in_kind" class="text-indigo-600" {{ old('donation_type', $donation->donation_type) == 'in_kind' ? 'checked' : '' }}>
            ...
            <div id="cash-fields" class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <input type="text" name="payment_reference" id="payment_reference" value="{{ old('payment_reference', $donation->payment_reference) }}"
                           class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    ...
                    <input type="date" name="payment_date" id="payment_date" value="{{ old('payment_date', $donation->payment_date?->format('Y-m-d')) }}" required
                           class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                </div>
                <div>
                ...
                <div>
                    ...
                    <select name="income_category_id" id="income_category_id" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                        ...
                        <option value="{{ $category->id }}" {{ old('income_category_id', $donation->income_category_id) == $category->id ? 'selected' : '' }}>
                            {{ $category->name }}
                        </option>
                        @endforeach
                    </select>
    ...
    <div class="mt-6 flex justify-end space-x-3">
        <a href="{{ route('admin.donations.show', $donation) }}" class="px-4 py-2 border border-gray-300 rounded-md shadow-sm text-sm font-medium text-gray-700 bg-white hover:bg-gray-50">Cancel</a>
        <button type="submit" class="px-4 py-2 border border-transparent rounded-md shadow-sm text-sm font-medium text-white bg-indigo-600 hover:bg-indigo-700">Update Donation</button>
    </div>
</form>
